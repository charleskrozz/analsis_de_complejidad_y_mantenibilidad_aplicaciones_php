...
use app\models\PorPagoVencimientoCustodiaSearch;
...
/**
 * PorpagovencimientocustodiaController implements the CRUD actions for PorPagoVencimientoCustodia model.
 */
class PorpagovencimientocustodiaController extends Controller
{
    ...
    /**
     * Lists all PorPagoVencimientoCustodia models.
     * @return mixed
     */
    ...
    /**
     * Displays a single PorPagoVencimientoCustodia model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new PorPagoVencimientoCustodia model.
    ...
    public function actionCreate()
    {
        if(Yii::$app->request->isAjax){
           $model = new PorPagoVencimientoCustodia();
    ...
    /**
     * Updates an existing PorPagoVencimientoCustodia model.
    ...
    /**
     * Finds the PorPagoVencimientoCustodia model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return PorPagoVencimientoCustodia the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = PorPagoVencimientoCustodia::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionMostrar(){
        $searchModel = new PorPagoVencimientoCustodiaSearch();
        $portVendidosCustodia= $searchModel->mostrarListado();
        return Json::encode($portVendidosCustodia);
    }
     public function actionConsultarfecha(){
        ...
        $PortafolioId = $session->get('PortafolioId');
          $searchModel = new PorPagoVencimientoCustodiaSearch();
        $portVendidosCustodia= $searchModel->consultarFecha($fecha,$fechaEnd,$PortafolioId);
        return Json::encode($portVendidosCustodia);
    }
    public function actionExportar($ids, $fech = "", $fech2 = "",$ext="") {
        ...
        $arrInfo = [];       
         $arrInfo['cols'] = ['Emisor', 'Numeracion', 'Tipo', "Suma0" => 'ValorNominal','Observaciones'];
                $arrInfo['plantilla'] = 'plantilla/PagosParcialesDeVencimientosNoRegistradosEnCustodia.xlsx';
                $arrInfo['nombreExcel'] = 'tmp/PagosParcialesDeVencimientosNoRegistradosEnCustodia.xlsx';
                $arrInfo['nombrePdf'] = 'tmp/PagosParcialesDeVencimientosNoRegistradosEnCustodia.pdf';
                $arrInfo['inicio'] = 10;
                $arrInfo['colInicio'] = 2;
